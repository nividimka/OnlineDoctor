package dkgroup.kz.onlinedoctor.presentation.appointment.patient.upcoming
...
@InjectViewState
class PatientUpcomingAppointmentPresenter @Inject constructor(
        private val router: FlowRouter,
        private val firestore: FirebaseFirestore,
        private val firebaseUser: FirebaseUser?
) : BasePresenter<PatientUpcomingAppointmentView>() {
    var medCard: MedCard? = null
    var appointments: MutableList<Appointment> = ArrayList()

    override fun attachView(view: PatientUpcomingAppointmentView?) {
        super.attachView(view)
        Log.e("attach", "attach")
        if (firebaseUser != null) {
            firestore.collection("appointment")
                    .whereEqualTo("patientEmail", firebaseUser.email)
