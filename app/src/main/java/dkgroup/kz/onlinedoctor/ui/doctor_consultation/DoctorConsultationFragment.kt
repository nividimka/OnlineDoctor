...
class DoctorConsultationFragment : Fragment(), OnlineConsultationView {
    ///@brief адаптер для табов, определяет в каком месте будет открываться определенный фрагмент, а так же их названия
    internal lateinit var adapter: DoctorConsultationViewPagerAdapter
    ...
    override fun onCreateView(inflater: LayoutInflater?, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view = inflater!!.inflate(R.layout.online_consultation_fragment, container, false)
        adapter = DoctorConsultationViewPagerAdapter(childFragmentManager)
    ...
    companion object {
        ...
        fun newInstance(args: Bundle): DoctorConsultationFragment {
            val fragment = DoctorConsultationFragment()
